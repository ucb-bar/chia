...
(``head`` + ``head_worker``, or ``ec2_worker`` + ``ec2_docker``) still
get their own row/column — the sweep verifies each logical worker
independently.
...
MACHINE_TAGS = ["head", "head_worker", "tailscale_worker", "ec2_worker",
                "ec2_docker"]
