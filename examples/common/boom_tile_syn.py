...
Usage:
    ray job submit --working-dir . -- python common/boom_tile_syn.py
"""
...
@ChiaFunction(resources={"VLSI": 1, "Syn": 0})  # 1 VLSI resource per synthesis run
def run_boom_tile_synthesis(
    generated_src_files: list[tuple[str, str]],
    vlsi_top: str,
    obj_dir: str,
    timeout_seconds: int = 172800,  # 48 hours
    cacti_sram_libs: list[dict[str, str]] | None = None,
    sky130_col_path: str = "/path/to/sky130_col",
) -> SynthesisResult:
    """Run synthesis with a persistent obj_dir on the shared filesystem.

    Unlike run_synthesis_for_module which uses a temp dir, this writes the
    full Hammer/Genus working directory (including syn-rundir) to obj_dir
    on /scratch so it survives after the task completes.
    """
...
def main(
    opts_dir: Path = Path("/path/to/opts_to_syn"),
    # Base mems.conf from the Chipyard MegaBoomChiaBigCacheConfig build
    base_mems_conf_path: str = (
        "/path/to/chipyard/sims/verilator/generated-src/"
        "chipyard.harness.TestHarness.MegaBoomChiaVerilatorBigCacheConfig/"
        "chipyard.harness.TestHarness.MegaBoomChiaVerilatorBigCacheConfig.top.mems.conf"
    ),
    mems_conf_inject_name: str = (
