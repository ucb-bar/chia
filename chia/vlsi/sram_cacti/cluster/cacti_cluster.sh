#!/usr/bin/env bash
#
# Bring the local cacti test cluster up/down using YOUR currently-active
...
#   ./cacti_cluster.sh up       # also: up --dry-run
#   ./cacti_cluster.sh down
#
...
REPO_ROOT="$(cd "$HERE/../../../.." && pwd)"   # .../chia/vlsi/sram_cacti/cluster -> repo root
YAML="$HERE/cacti_local.yaml"
...
echo "[cacti-cluster] head env activation: ${CHIA_HEAD_ENV_SETUP}"
